# Databento Options Data Pull — Wider Strike Band

Two scripts that pull a wider band of option strikes from Databento, covering
±3 weeks of spot drift around each earnings event (`earn_dt` and `next_earn`).
Same dataset/schema as before (`OPRA.PILLAR`, `cbbo-1m`), same expiries — only
the strike coverage is widened.

## Run order

```bash
# 1. Free cost preview (calls Databento metadata.get_cost only)
...
Knobs at the top of `01b_cost_estimate_wide.py`:
...
- `data/symbol_list_wide.csv` — written by 01b, consumed by 02b
- `data/options_daily_wide.parquet` — written by 02b, columns: `symbol, date, bid, ask, mid`

The original `data/options_daily.parquet` is **not** touched.

## Dependencies

- `pipeline/config.py` — must define `DATABENTO_KEY`, `BACKTEST_START`, `BACKTEST_END`, `DATASET`, `SCHEMA`, `EOD_UTC_HOUR`, `DATA_DIR`
- `pipeline/options_utils.py` — must export `atm_strike`, `osi_symbol`
- `data/trade_schedule.csv`, `data/equity_prices.parquet` — already present from the original pipeline
- Python: `pandas`, `numpy`, `databento`
- Env var: `DATABENTO_API_KEY`

---

## `pipeline/01b_cost_estimate_wide.py`
...
## `pipeline/02b_pull_data_wide.py`
